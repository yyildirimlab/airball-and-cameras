    ...
    def update(self):
        ...
        if (bge.logic.keyboard.inputs[bge.events.ESCKEY].values[0]) or (self.elapsed_time > self.duration):
            ...
            if self.object["use_keyboard"] or self.object["mouse_debug"]:
                ...
                x = (movement[3] - V_ZERO_OFFSET["X"]) / V_ZERO_OFFSET["X"]
                y = (movement[2] - V_ZERO_OFFSET["Y"]) / V_ZERO_OFFSET["Y"]
                z = (movement[1] - V_ZERO_OFFSET["Z"]) / V_ZERO_OFFSET["Z"]
                if abs(x) < V_THRESHOLD["X"]:
                    x = 0
                if abs(y) < V_THRESHOLD["Y"]:
                    y = 0
                if abs(z) < V_THRESHOLD["Z"]:
                    z = 0
                z = z*2.9
                x = x*1.4
                apply_offset = [x * -0.5 * ydir, y * -0.5, z * 0.01]

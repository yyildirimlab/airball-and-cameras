...
class Component(bge.types.KX_PythonComponent):
        ...
        try:
            ...
            if hasattr(self, 'laser') and self.laser is not None:
                try:
                    ...
                    print(f"Error cleaning up laser: {e}")
                finally:
                    del self.laser
        
            # Clean up NI-DAQ tasks
            if not self.object["mouse_debug"]:
                try:
                    # turn off cameras
                    if hasattr(self, 'task_do_camera') and self.task_do_camera is not None:
                        self.task_do_camera.write(False)
                        self.task_do_camera.close()
                        del self.task_do_camera

                    # turn off laser
                    if hasattr(self, 'task_do_laser') and self.task_do_laser is not None:
                        self.task_do_laser.write(False)
                        self.task_do_laser.close()
                        del self.task_do_laser
                    
                    # close nidaqmx task
                    if hasattr(self, 'task_ai_airball') and self.task_ai_airball is not None:
                        self.task_ai_airball.stop()
                        self.task_ai_airball.close()
                        del self.task_ai_airball

                except Exception as e:
                    print(f"Error during NI-DAQ cleanup: {e}")

            # Clean up any remaining game object references
            if hasattr(self, 'object'):
                try:
                    # Remove any custom properties
                    for prop in list(self.object.keys()):
                        del self.object[prop]
                except Exception as e:
                    print(f"Error cleaning up game object properties: {e}")

        except Exception as e:
            print(f"Error during overall cleanup: {e}")
    ...
    def update(self):
        ...
        self.pressed_escape = bge.logic.keyboard.inputs[bge.events.ESCKEY].values[0]
        self.duration_met   = not self.simulated and (self.elapsed_time > self.duration)

        if self.abort or self.pressed_escape or self.duration_met:
    ...
    def read_non_blocking(self):
        """Non-blocking read of available samples from the NI-DAQ task."""
        num_available = self.task_ai_airball.in_stream.avail_samp_per_chan
        if num_available > 0:
            # Read only the most recent sample
            samples = self.task_ai_airball.read(number_of_samples_per_channel=num_available)
            # Use the latest sample for movement
            latest_sample = [samples[0][-1], samples[1][-1], samples[2][-1]]
    ...
    def startNidaq(self):
        ...
        # Configure for continuous acquisition with non-blocking reads
        self.task_ai_airball.timing.cfg_samp_clk_timing(
            rate=60,  # High sample rate to ensure we always have fresh data
            sample_mode=AcquisitionType.CONTINUOUS,
            samps_per_chan=30  # Large buffer to prevent overflows
        )
